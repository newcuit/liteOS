...
* ReturnValue    : 返回None
**************************************************************************************/
static void __used backtrace(uint32_t *stack,  uint32_t is_kernel)
{
    uint32_t depth;
#ifdef USING_OS_FREERTOS
    uint8_t *tcb_head = NULL;
    uint32_t *usr_stack = NULL;
    uint32_t *usr_stack_top = NULL;
#endif
    uint32_t psr;
...
#endif
            stack+depth, regs.r0, regs.r1, regs.r2, regs.r3, regs.pc, regs.r12, regs.lr, regs.psr, psr
#ifdef USING_OS_FREERTOS
            ,usr_stack, tcb_head, usr_stack_top, *usr_stack_top, *(usr_stack_top+1), *(usr_stack_top+2),
            *(usr_stack_top+3)
#endif
			);
