...
* ReturnValue    : 返回错误码
**************************************************************************************/
int8_t __default local_irq_disable(void)
{
    return 0;
}
...
* ReturnValue    : 返回错误码
**************************************************************************************/
int8_t __default local_irq_enable(void)
{
    return 0;
}
...
* ReturnValue    : 返回错误码
**************************************************************************************/
int32_t __default irq_fast_dispatch(uint32_t irq)
{
    ...
    list_for_each_entry_safe(irq_reg, tmp, &irq_list, list) {
        if(irq_reg->irq == irq && irq_reg->handler) {
            irq_reg->handler(irq);
            if(irq_reg->trig & IRQ_TRIG_SHARED) continue;
            return 0;
        }
	}
...
* ReturnValue    : 返回错误码
**************************************************************************************/
int32_t __default request_irq(uint32_t irq, uint8_t trig, irq_cb_t handler)
{
    ...
    irq_set_trigger(irq, trig & (~IRQ_TRIG_SHARED));
    irq_enable(irq);
    irq_total++;
...
* ReturnValue    : 返回错误码
**************************************************************************************/
int32_t __default free_irq(uint32_t irq)
{
    struct irq_reg_s *irq_reg = NULL, *tmp;
